package huiiuh.com.driverfly;

/**
 * @author Admin
 * @version $Rev$
 * @des ${TODO}
 * @updateAuthor $Author$
 * @updateDes ${TODO}
 */
public class Contact {

    public static final String LISTNUM = "listnum";
    public static final String ISTEST = "sitest";
    public static final String QUESTION_TYPE = "question_type";
    public static String basepath = "https://way.jd.com/jisuapi/driverexamQuery?";
    public static String type = "type=";
    public static String subject = "&subject=";
    public static String pagesize = "&pagesize=20";
    public static String pagenum = "&pagenum=";
    public static String sort = "&sort=";
    public static String appkey = "&appkey=109a15e4cede86cab981bbf19bcf2621";

    //是不是第一次使用
    public static String ISUSED = "ISUSED";
    //是不是选择过车辆类型
    public static String ISCHOOSETYPE = "ISCHOOSETYPE";
    //选择的车辆类型 c1,a2,b1,d
    public static String CARTYPE = "CARTYPE";
    //是不是属于答题模式
    public static String ISDATI = "ISDATI";
    //测试模式（顺序出题，随机出题，考试模式）
    public static String TESTTYPE = "TESTTYPE";
    //记录测试的当前位置
    public static String CURRENTITEM = "CURRENTITEM";
    //字体大小
    public static String TEXTSIZE = "TEXTSIZE";
    //选择的科目
    public static String SUBJECT = "SUBJECT";

}
